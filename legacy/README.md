...
link is `modeling.build_3d_model`, which dispatches retired architecture kinds
...
| Folder | Contents |
|---|---|
| `arch/` | Retired 3-D architectures: standalone `SirenFNO3d`, and the plain `neuralop` FNO. Reached via `modeling.build_3d_model` for `kind="sirenfno"` / `"fno"`. |
| `xhi2d/` | The complete 2-D x_HI pipeline — trainer, `SliceCache` and its builders, visualization, and the contrast/sharpness analysis that produced `NOTES-contrast-map.md`. |
...
| `lploss.py` | Unused; kept only because it predates the vendored `neuralop` copy. |
...
- `dataset/dataset.py` → `legacy/xhi2d/dataset.py`; its `make_file_split` was a
  byte-identical copy of the 3-D one and is now re-exported from
...
- `siren_fno_3d.py` was split: the SIREN weight generators the operator registry
  needs stayed in `siren.py`, the standalone architecture moved to
